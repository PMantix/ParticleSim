    ...
    for step in 0usize..steps {
        ...
        if step % 50 == 0 {
            let avg_last_10 = if step >= 10 {
                step_times[step.saturating_sub(9)..=step]
                    .iter()
                    .sum::<u128>()
                    / 10
            } else {
            ...
            println!(
                "Step {}: {:.2}ms (avg last 10: {:.2}ms)",
                step,
                duration.as_millis(),
                avg_last_10 as f64 / 1000.0
            );
        }
    }
...
fn main() {
    ...
    let few_particles_slowdown =
        test_simulation_performance("50 particles, 200 steps", sim_few, 200);

    println!("\n=== ANALYSIS ===");
    println!("Short run (50 steps):     {:.2}x slowdown", short_slowdown);
    println!("Long run (200 steps):     {:.2}x slowdown", long_slowdown);
    println!(
        "Few particles (50):       {:.2}x slowdown",
        few_particles_slowdown
    );
