...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Domain: {}×{} Å = {:.0} Å²",
        domain_width, domain_height, total_domain_area
    );
    ...
    println!(
        "Solid fraction: {:.1}%",
        (total_solid_area / total_domain_area) * 100.0
    );
    ...
    println!(
        "Liquid region density: {:.6} particles/Å²",
        liquid_density_2d
    );
    println!(
        "Overall domain density: {:.6} particles/Å²",
        overall_density_2d
    );
    ...
    println!(
        "Area per particle (liquid region): {:.1} Å²",
        area_per_particle_liquid
    );
    println!(
        "Area per particle (overall domain): {:.1} Å²",
        area_per_particle_overall
    );
    ...
    println!(
        "Typical spacing (liquid region): {:.1} Å",
        typical_spacing_liquid
    );
    println!(
        "Typical spacing (overall domain): {:.1} Å",
        typical_spacing_overall
    );
    ...
    println!(
        "Particle radii: Li⁺={:.2} Å, Anion={:.1} Å, EC={:.1} Å, DMC={:.1} Å",
        li_radius, anion_radius, ec_radius, dmc_radius
    );

    let total_particle_area = li_count as f32 * li_area
        + anion_count as f32 * anion_area
        + ec_count as f32 * ec_area
        + dmc_count as f32 * dmc_area;

    println!(
        "Total particle cross-sectional area: {:.0} Å²",
        total_particle_area
    );
    ...
    println!(
        "Packing fraction (liquid region): {:.1}%",
        packing_fraction_liquid * 100.0
    );
    println!(
        "Packing fraction (overall domain): {:.1}%",
        packing_fraction_overall * 100.0
    );
    ...
    println!(
        "Molecular masses: Li⁺={:.1} amu, PF6⁻={:.1} amu, EC={:.1} amu, DMC={:.1} amu",
        li_mass, pf6_mass, ec_mass, dmc_mass
    );

    // Total mass in simulation (amu)
    let total_mass_amu = li_count as f32 * li_mass
        + anion_count as f32 * pf6_mass
        + ec_count as f32 * ec_mass
        + dmc_count as f32 * dmc_mass;
    ...
    println!(
        "Total mass in simulation: {:.0} amu = {:.2e} g",
        total_mass_amu, total_mass_g
    );
    ...
    println!("\n2D Mass Densities:");
    println!(
        "Liquid region: {:.2e} g/cm² in {:.2e} cm²",
        mass_density_2d_liquid, liquid_area_cm2
    );
    println!(
        "Overall domain: {:.2e} g/cm² in {:.2e} cm²",
        mass_density_2d_overall, overall_area_cm2
    );
    ...
    for density_3d in realistic_3d_densities.iter() {
        for thickness in reference_thicknesses.iter() {
            ...
            println!(
                "  {:.2} g/cm³ × {:.0}Å = {:.2e} g/cm² → Our ratio: {:.1}×",
    ...
    for thickness in reference_thicknesses.iter() {
        ...
        println!(
            "  At {:.0}Å thickness: {:.2} g/cm³",
            thickness, implied_3d_density
        );
    ...
    let weighted_avg_radius = (li_count as f32 * li_radius
        + anion_count as f32 * anion_radius
        + ec_count as f32 * ec_radius
        + dmc_count as f32 * dmc_radius)
        / total_liquid_particles as f32;
    ...
    println!(
        "Spacing/diameter ratio (liquid): {:.1}",
        spacing_diameter_ratio_liquid
    );
    println!(
        "Spacing/diameter ratio (overall): {:.1}",
        spacing_diameter_ratio_overall
    );
    ...
    if anion_count > 0 {
        println!(
            "Li⁺:Anion ratio: {:.2}:1",
            li_count as f32 / anion_count as f32
        );
    }
    if dmc_count > 0 {
    ...
    println!(
        "Mole fractions: Li⁺={:.3}, Anions={:.3}, EC={:.3}, DMC={:.3}",
    ...
    println!(
        "Liquid particles: {} in {:.0} Å² liquid area",
        total_liquid_particles, available_liquid_area
    );
    println!(
        "Liquid density: {:.4} particles/Å² ({:.1}% packing)",
        liquid_density_2d,
        packing_fraction_liquid * 100.0
    );
    println!(
        "Spacing/diameter: {:.1} (liquid)",
        spacing_diameter_ratio_liquid
    );
