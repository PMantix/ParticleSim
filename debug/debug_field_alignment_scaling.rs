...
fn main() {
    ...
    let mut bodies = vec![
        body::Body::new_from_species(
            Vec2::new(-10.0, 0.0),
            Vec2::zero(),
            -1.0,
            body::Species::LithiumMetal,
        ), // Source: will have extra electrons
        body::Body::new_from_species(
            Vec2::new(10.0, 0.0),
            Vec2::zero(),
            1.0,
            body::Species::LithiumMetal,
        ), // Destination: will be electron-deficient
    ];
...
fn test_hopping_scenario(
    ...
    let local_field =
        external_field + quadtree.field_at_point(&bodies, src_body.pos, config.coulomb_constant);
    ...
    println!(
        "  {}: External field: {:.3}, {:.3}",
        label, external_field.x, external_field.y
    );
    println!(
        "  Local field: {:.3}, {:.3} (mag: {:.3})",
        local_field.x,
        local_field.y,
        local_field.mag()
    );
