...
fn main() {
    ...
    for step in 0..100 {
        ...
        if step % 10 == 0 {
            // Hardcoded since we removed the constant
        ...
        if step % 20 == 0 {
            println!(
                "Step {}: Total={:.2}ms | Forces={:.2}ms | History={:.2}ms | History%={:.1}%",
                step,
    ...
    let history_slowdown = if early_history > 0 {
        late_history as f64 / early_history as f64
    } else {
        1.0
    };
    ...
    println!("\n=== BOTTLENECK ANALYSIS ===");
    println!(
        "Total step slowdown: {:.2}x ({:.2}ms → {:.2}ms)",
        total_slowdown,
        early_total as f64 / 1000.0,
        late_total as f64 / 1000.0
    );
    println!(
        "History operation slowdown: {:.2}x ({:.2}ms → {:.2}ms)",
        history_slowdown,
        early_history as f64 / 1000.0,
        late_history as f64 / 1000.0
    );

    let avg_history_percent = (history_times.iter().sum::<u128>() as f64
        / step_times.iter().sum::<u128>() as f64)
        * 100.0;
    println!(
        "History operations consume {:.1}% of total step time on average",
        avg_history_percent
    );
