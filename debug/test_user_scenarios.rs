...
fn run_performance_test(sim: &mut simulation::Simulation, scenario_name: &str) {
    ...
    for step in 0..150 {
        // Longer test to see progressive issues
    ...
    println!(
        "    Early: {:.2}ms, Mid: {:.2}ms, Late: {:.2}ms",
        early_avg as f64 / 1000.0,
        mid_avg as f64 / 1000.0,
        late_avg as f64 / 1000.0
    );
    println!(
        "    Early→Mid: {:.2}x, Early→Late: {:.2}x",
        early_to_mid, early_to_late
    );
