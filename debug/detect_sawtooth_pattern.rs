...
fn main() {
    ...
    for step in 0..2000 {
        ...
        if step % 100 == 0 && step > 0 {
            ...
            println!(
                "Step {}: {:.2}ms | Keyframes: {} | Deltas: {} | Frame: {}",
                step,
                recent_avg as f64 / 1000.0,
                stats.keyframe_count,
                stats.delta_count,
                sim.frame
            );
    ...
    println!(
        ...
        step_times.iter().sum::<u128>() as f64 / step_times.len() as f64 / 1000.0
    );
}
...
fn analyze_sawtooth_pattern(step_times: &[u128], history_stats: &[(usize, usize, usize, u128)]) {
    ...
    for chunk_start in (0..step_times.len()).step_by(chunk_size) {
        ...
        if chunk_start % 500 == 0 {
            println!(
                "Chunk {}-{}: Avg={:.2}ms, Min={:.2}ms, Max={:.2}ms, Variation={:.1}%",
                chunk_start,
                chunk_end - 1,
    ...
    if valleys.len() > 2 && peaks.len() > 2 {
        ...
        let valley_intervals: Vec<usize> = valleys.windows(2).map(|w| w[1].0 - w[0].0).collect();
        ...
        if !valley_intervals.is_empty() {
            ...
            println!(
                "Estimated cycle time: {:.1}s (if running at 60 FPS)",
                cycle_time_estimate
            );
            ...
            if avg_cycle > 50 && avg_cycle < 2000 {
                ...
                println!(
                    "   This confirms the compressed history system is causing periodic slowdowns."
                );
    ...
    if history_stats.len() > 10 {
        ...
        let low_delta_avg = delta_perf_correlation[..delta_perf_correlation.len() / 3]
            ...
            .map(|(_, time)| *time)
            .sum::<u128>()
            / (delta_perf_correlation.len() / 3) as u128;
        ...
        println!(
            "Low delta count performance: {:.2}ms",
            low_delta_avg as f64 / 1000.0
        );
        println!(
            "High delta count performance: {:.2}ms",
            high_delta_avg as f64 / 1000.0
        );
