...
pub fn run() {
    ...
    let threads = std::thread::available_parallelism().unwrap().get().max(crate::config::MIN_THREADS) - crate::config::THREADS_LEAVE_FREE;
    ...
    let config = quarkstrom::Config {
        window_mode: quarkstrom::WindowMode::Windowed(crate::config::WINDOW_WIDTH, crate::config::WINDOW_HEIGHT),
    };
    ...
    *SIM_COMMAND_SENDER.lock() = Some(tx);
    ...
    if let Some(ref sim_config) = init_config.simulation {
        if let Some(domain_bounds) = sim_config.domain_bounds {
            ...
            tx.send(SimCommand::SetDomainSize { width: domain_size, height: domain_size }).unwrap();
        }
    }
    ...
    for circle_config in &init_config.particles.circles {
        match circle_config.to_species() {
            Ok(species) => {
                ...
                tx.send(SimCommand::AddCircle { body, x: circle_config.x, y: circle_config.y, radius: circle_config.radius }).unwrap();
                println!("Added circle: {} at ({}, {}) with radius {}", circle_config.species, circle_config.x, circle_config.y, circle_config.radius);
            }
    ...
    for rect_config in &init_config.particles.metal_rectangles {
        match rect_config.to_species() {
            Ok(species) => {
                ...
                tx.send(SimCommand::AddRectangle { body, x: origin_x, y: origin_y, width: rect_config.width, height: rect_config.height }).unwrap();
                println!("Added {} rectangle: {}x{} at center ({}, {})", rect_config.species, rect_config.width, rect_config.height, rect_config.x, rect_config.y);
            }
    ...
    for foil_config in &init_config.particles.foil_rectangles {
        ...
        tx.send(SimCommand::AddFoil { width: foil_config.width, height: foil_config.height, x: origin_x, y: origin_y, particle_radius: Species::FoilMetal.radius(), current: foil_config.current }).unwrap();
        println!("Added foil: {}x{} at center ({}, {}) with current {}", foil_config.width, foil_config.height, foil_config.x, foil_config.y, foil_config.current);
    }
    ...
    for random_config in &init_config.particles.random {
        match random_config.to_species() {
            Ok(species) => {
                ...
                tx.send(SimCommand::AddRandom { body, count: random_config.count, domain_width: random_config.domain_width, domain_height: random_config.domain_height }).unwrap();
                println!("Added {} random {} particles in {}x{} domain", random_config.count, random_config.species, random_config.domain_width, random_config.domain_height);
            }
