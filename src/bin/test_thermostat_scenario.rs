...
fn main() {
    ...
    if let Some(cfg) = &init_config {
        println!("Loaded init_config.toml (optional)");
        if let Some(simc) = &cfg.simulation {
            let (w, h) = simc.domain_size();
            println!("  Domain: {}x{}", w, h);
        }
    }
    ...
    match scenario::load_and_apply_scenario() {
        ...
        Err(e) => {
            println!(
                "Scenario load failed ({}), attempting hardcoded fallback",
                e
            );
    ...
    for step in 1..=50usize {
        ...
        if step % 10 == 0 {
            ...
            println!(
                "  Liquid T: {:.2}K | Global T: {:.2}K | Δ={:.2}K",
                liquid_temp,
                global_temp,
                liquid_temp - 300.0
            );
    ...
    println!(
        "  Deviation (liquid-target): {:.2}K",
        final_liquid_temp - 300.0
    );
    if (final_liquid_temp - 300.0).abs() < 5.0 {
        println!("✅ Within 5K of target");
    } else {
        println!("❌ Deviation {:.2}K", (final_liquid_temp - 300.0).abs());
    }
}
...
fn analyze_particles(sim: &Simulation) {
    ...
    let solvent_count =
        counts.get(&Species::EC).unwrap_or(&0) + counts.get(&Species::DMC).unwrap_or(&0);
