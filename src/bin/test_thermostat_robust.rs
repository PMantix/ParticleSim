...
fn main() {
    ...
    println!("This test loads the actual default scenario and tracks thermostat behavior in real-time\n");
    ...
    for step in 1..=20 {
        ...
        println!("\n--- STEP {} ---", step);
        println!("Temperature: {:.3}K -> {:.3}K (Δ{:+.3}K)", 
            temp_before.solvent_temp, temp_after.solvent_temp,
            temp_after.solvent_temp - temp_before.solvent_temp);
        
        println!("Avg velocity magnitude:");
        println!("  EC:  {:.3} -> {:.3} (Δ{:+.3})", 
            velocities_before.ec_avg, velocities_after.ec_avg,
            velocities_after.ec_avg - velocities_before.ec_avg);
        println!("  DMC: {:.3} -> {:.3} (Δ{:+.3})", 
            velocities_before.dmc_avg, velocities_after.dmc_avg,
            velocities_after.dmc_avg - velocities_before.dmc_avg);
        println!("  Li+: {:.3} -> {:.3} (Δ{:+.3})", 
            velocities_before.li_avg, velocities_after.li_avg,
        ...
        if should_activate {
            println!("  ⚙️ Thermostat should activate at frame {} (t={:.1}fs)", sim.frame, current_time);
        } else {
            println!("  ⏸️ Thermostat inactive at frame {} (t={:.1}fs)", sim.frame, current_time);
        }
...
fn create_realistic_simulation(init_config: &InitConfig) -> Simulation {
    ...
    config.temperature = init_config.simulation.as_ref()
...
fn populate_simulation_from_config(sim: &mut Simulation, init_config: &InitConfig) {
    ...
    for circle_config in &init_config.particles.circles {
        if let Ok(species) = circle_config.to_species() {
            ...
            println!("Adding {} {} particles in circle at ({}, {}) radius {}", 
                particle_count, format!("{:?}", species), circle_config.x, circle_config.y, radius);
    ...
    for rect_config in &init_config.particles.metal_rectangles {
        if let Ok(species) = rect_config.to_species() {
            ...
            let particle_count = (rect_config.width * rect_config.height * particle_density) as usize;
            
            println!("Adding {} {} particles in rectangle {}×{} at ({}, {})", 
                particle_count, format!("{:?}", species), rect_config.width, rect_config.height,
    ...
    for random_config in &init_config.particles.random {
        if let Ok(species) = random_config.to_species() {
            ...
            println!("Adding {} random {} particles", random_config.count, format!("{:?}", species));
...
fn count_solvent_particles(bodies: &[Body]) -> usize {
    bodies.iter().filter(|b| matches!(b.species, Species::EC | Species::DMC)).count()
}
...
fn calculate_detailed_temperature(bodies: &[Body]) -> DetailedTemp {
    ...
    DetailedTemp {
        total_temp: if total_count > 0 { total_ke / total_count as f32 } else { 0.0 },
        solvent_temp: if solvent_count > 0 { solvent_ke / solvent_count as f32 } else { 0.0 },
        ec_temp: if ec_count > 0 { ec_ke / ec_count as f32 } else { 0.0 },
        dmc_temp: if dmc_count > 0 { dmc_ke / dmc_count as f32 } else { 0.0 },
        li_temp: if li_count > 0 { li_ke / li_count as f32 } else { 0.0 },
        solvent_count,
    }
}
...
fn capture_velocity_stats(bodies: &[Body]) -> VelocityStats {
    ...
    VelocityStats {
        ec_avg: if ec_count > 0 { ec_vel_sum / ec_count as f32 } else { 0.0 },
        dmc_avg: if dmc_count > 0 { dmc_vel_sum / dmc_count as f32 } else { 0.0 },
        li_avg: if li_count > 0 { li_vel_sum / li_count as f32 } else { 0.0 },
    }
}
...
fn analyze_detailed_state(sim: &Simulation, step: u32) {
    ...
    println!("    Solvent (EC+DMC): {:.3}K ({} particles)", temp.solvent_temp, temp.solvent_count);
