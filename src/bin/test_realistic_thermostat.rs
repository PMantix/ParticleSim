...
fn main() {
    ...
    sim.config.temperature = init_config
        .simulation
        .as_ref()
    ...
    println!(
        "  Thermostat interval: {}fs",
        sim.config.thermostat_interval_fs
    );
    ...
    println!("\n=== RESULT ===");
    println!(
        "Target: {:.1}K, Final: {:.3}K, Deviation: {:.3}K",
        target, final_temp, deviation
    );
...
fn analyze_state(sim: &Simulation, step: u32) {
    ...
    let ec_count = sim
        .bodies
        .iter()
        .filter(|b| matches!(b.species, Species::EC))
        .count();
    let dmc_count = sim
        .bodies
        .iter()
        .filter(|b| matches!(b.species, Species::DMC))
        .count();
    let li_count = sim
        .bodies
        .iter()
        .filter(|b| matches!(b.species, Species::LithiumIon))
        .count();
    let metal_count = sim
        .bodies
        .iter()
        .filter(|b| matches!(b.species, Species::LithiumMetal))
        .count();

    println!(
        "Step {} - Total: {}, EC: {}, DMC: {}, Li+: {}, Metal: {}",
        step,
        sim.bodies.len(),
        ec_count,
        dmc_count,
