//! E.2 Repeatability with dendrites: 3 consecutive rest→pulse→rest cycles
//! on a cell with LithiumMetal dendrite chains protruding from exposed electrode faces.
//!
//! Protocol per cycle:
...
/// Add 2-wide LithiumMetal dendrite chains protruding from 3 exposed faces of both electrodes.
/// 5 chains per face, 5 particles per chain, 2 particles wide, center-to-center = diameter.
fn add_dendrites(sim: &mut Simulation) -> usize {
    let r_li = Species::LithiumMetal.radius(); // 1.52
    let d_li = r_li * 2.0;                     // 3.04
    let chains_per_face = 5;
    let particles_per_chain = 5;
    let face_len = 80.0_f32;
    let spacing = face_len / (chains_per_face as f32 + 1.0);
    ...
    // Width offsets: two rows offset ±r_li perpendicular to chain direction
    let width_offsets = [-r_li, r_li];

    for chain_idx in 1..=chains_per_face {
        let frac = chain_idx as f32 * spacing;

        // --- Left electrode ---
        // Right face: chains grow rightward (x), 2-wide in y
        {
            let root_y = bottom_y + frac;
            for &dy in &width_offsets {
                for p in 0..particles_per_chain {
                    let x = left_exposed_x + r_li + (p as f32) * d_li;
                    positions.push(Vec2::new(x, root_y + dy));
                }
            }
        }
        // Top face: chains grow upward (y), 2-wide in x
        {
            let root_x = -140.0 + frac;
            for &dx in &width_offsets {
                for p in 0..particles_per_chain {
                    let y = top_y + r_li + (p as f32) * d_li;
                    positions.push(Vec2::new(root_x + dx, y));
                }
            }
        }
        // Bottom face: chains grow downward (y), 2-wide in x
        {
            let root_x = -140.0 + frac;
            for &dx in &width_offsets {
                for p in 0..particles_per_chain {
                    let y = bottom_y - r_li - (p as f32) * d_li;
                    positions.push(Vec2::new(root_x + dx, y));
                }
            }
        }
        ...
        // --- Right electrode ---
        // Left face: chains grow leftward (x), 2-wide in y
        {
            let root_y = bottom_y + frac;
            for &dy in &width_offsets {
                for p in 0..particles_per_chain {
                    let x = right_exposed_x - r_li - (p as f32) * d_li;
                    positions.push(Vec2::new(x, root_y + dy));
                }
            }
        }
...
fn main() {
    ...
    eprintln!("E.2 Dendrite: ratio={:.3}, {} pulses, pulse={:.0}fs, rest={:.0}fs",
    ...
    // Add dendrite chains before equilibration
    let dendrite_count = add_dendrites(&mut sim);
    eprintln!("Added {} dendrite particles (5 chains x 5 particles x 6 faces)", dendrite_count);
