...
impl Simulation {
    ...
    pub fn apply_thermostat(&mut self) {
        ...
        tdbg!("[thermostat-enter] frame={} target_temp={} bodies_count={}", self.frame, target_temp, self.bodies.len());
    ...
    let mut _anion_count = 0; // debug only
        ...
        tdbg!("[thermostat-debug] Li+: {} Anion: {} EC: {} DMC: {} Total bodies: {}", _li_count, _anion_count, _ec_count, _dmc_count, self.bodies.len());
        if liquid_count == 0 { 
    ...
    if current_temp < 10.0 {
            ...
            for body in &self.bodies {
                ...
                tdbg!("[thermostat-sample] particle={} species={:?} mass={:.6} vel_mag={:.6} ke={:.6}", 
                    ...
                    debug_count += 1;
                    if debug_count >= 5 && debug_count >= liquid_count { break; }
                }
            }
            let avg_vel_mag = if debug_count > 0 { total_vel_mag / debug_count as f32 } else { 0.0 };
            let avg_mass = if debug_count > 0 { total_mass / debug_count as f32 } else { 0.0 };
            tdbg!("[thermostat-avg] avg_vel_mag={:.6} avg_mass={:.6} sampled={}", avg_vel_mag, avg_mass, debug_count);
        }
        ...
        tdbg!("[thermostat-active] Thermostat will run: current={:.6}K target={:.2}K", current_temp, target_temp);
        if current_temp <= 1e-3 { // effectively zero Kelvin
            ...
            tdbg!("[thermostat-bootstrap] frame={} bootstrapped velocities", self.frame);
