    ...
    pub fn target_memory_mb(mut self, target_mb: f64, particle_count: usize) -> Self {
        ...
        if current_total_mb > target_mb {
            ...
            self.keyframe_interval =
                (self.keyframe_interval as f64 / reduction_factor.sqrt()) as usize;
            self.max_keyframes =
                ((self.max_keyframes as f64 * reduction_factor.sqrt()) as usize).max(5);
...
impl CompressedHistorySystem {
    ...
    fn create_keyframe(&mut self, snapshot: LightSnapshot) {
        ...
        while self.keyframes.len() > self.config.max_keyframes {
            ...
            self.deltas
                .retain(|delta| delta.keyframe_frame != removed.frame);
        }
    }
    ...
    fn generate_delta(
        ...
        let thermostat_changed =
            (current.last_thermostat_time - previous.last_thermostat_time).abs() > 1e-6;
        ...
        if body_deltas.is_empty()
            && foil_deltas.is_empty()
            && !dt_changed
            && !thermostat_changed
            && !config_changed
            && !domain_changed
        ...
        Some(DeltaSnapshot {
            ...
            last_thermostat_time: if thermostat_changed {
                Some(current.last_thermostat_time)
            } else {
                None
            },
            body_deltas,
            ...
            config: if config_changed {
                Some(current.config.clone())
            } else {
                None
            },
            domain_width: if domain_changed {
                Some(current.domain_width)
            } else {
                None
            },
            domain_height: if domain_changed {
                Some(current.domain_height)
            } else {
                None
            },
            domain_depth: if domain_changed {
                Some(current.domain_depth)
            } else {
                None
            },
    ...
    fn cleanup_old_data(&mut self) {
        ...
        if let Some(oldest_keyframe) = self.keyframes.front() {
            self.deltas
                .retain(|delta| delta.frame >= oldest_keyframe.frame);
        }
    }
        ...
        if let Some(base_keyframe) = self.keyframes.iter().rev().find(|kf| kf.frame <= frame) {
            ...
            while current_frame < frame {
                ...
                if !self
                    .deltas
                    .iter()
                    .any(|d| d.frame == current_frame && d.keyframe_frame <= base_keyframe.frame)
                {
                    return false;
                }
            }
    ...
    pub fn reconstruct_frame(&self, frame: usize) -> Result<LightSnapshot, ReconstructionError> {
        ...
        let mut deltas_to_apply: Vec<&DeltaEntry> = self
            .deltas
            .iter()
            .filter(|delta| {
        ...
        for delta_entry in &deltas_to_apply {
            ...
            for body_delta in &delta_entry.delta.body_deltas {
                if let Some(body) = reconstructed
                    .bodies
                    .iter_mut()
                    .find(|b| b.id == body_delta.id)
                {
                    self.apply_body_delta(body, body_delta)?;
                } else {
                    // New body - this shouldn't happen in normal operation
                    return Err(ReconstructionError::UnknownBodyId {
                        id: body_delta.id,
                        frame,
                    });
                }
            ...
            for foil_delta in &delta_entry.delta.foil_deltas {
                if let Some(foil) = reconstructed
                    .foils
                    .iter_mut()
                    .find(|f| f.id == foil_delta.id)
                {
                    self.apply_foil_delta(foil, foil_delta)?;
                } else {
                    return Err(ReconstructionError::UnknownFoilId {
                        id: foil_delta.id,
                        frame,
                    });
                }
    ...
    pub fn reconstruct_frames(
        &self,
        frames: &[usize],
    ) -> Result<Vec<LightSnapshot>, ReconstructionError> {
        ...
        for (&keyframe_frame, group_frames) in &frames_by_keyframe {
            ...
            for &frame in group_frames {
                let reconstructed = if frame == keyframe_frame {
                    ...
                    let deltas: Vec<&DeltaEntry> = self
                        .deltas
                        .iter()
        ...
        let frame_to_snapshot: HashMap<usize, LightSnapshot> = sorted_frames
            .iter()
    ...
    pub fn validate_reconstruction(
        &self,
        frame: usize,
        reconstructed: &LightSnapshot,
    ) -> Result<ValidationResult, ReconstructionError> {
        ...
        for body in &reconstructed.bodies {
            ...
            if body.charge.abs() > 10.0 {
                // More than 10 elementary charges is unusual
        ...
        for (&body_id, &foil_id) in &reconstructed.body_to_foil {
            if !reconstructed.bodies.iter().any(|b| b.id == body_id) {
                issues.push(ValidationIssue::OrphanedMapping { body_id, foil_id });
            }
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingDelta {
                ...
                write!(
                    f,
                    "Missing delta for frame {} (reconstructing frame {})",
                    frame, target_frame
                )
            }
...
impl From<&LightFoil> for Foil {
    fn from(light: &LightFoil) -> Self {
        ...
        let overpotential_controller = if let (Some(target), Some(kp), Some(ki), Some(kd)) = (
            light.overpotential_target,
            light.overpotential_kp,
            light.overpotential_ki,
            light.overpotential_kd,
        ) {
