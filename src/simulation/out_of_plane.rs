...
fn enforce_metal_z_boundaries(sim: &mut Simulation, max_z: f32) {
    ...
    let any_metal = sim
        .bodies
        .iter()
        .any(|b| matches!(b.species, Species::LithiumMetal | Species::FoilMetal));
    if !any_metal {
        return;
    }
    ...
    if use_cell {
        let metal_max_r = Species::LithiumMetal
            .radius()
            .max(Species::FoilMetal.radius());
...
fn apply_many_body_z_forces(sim: &mut Simulation, _max_z: f32) {
    ...
    for i in 0..sim.bodies.len() {
        ...
        let neighbors = if use_cell {
            sim.cell_list
                .find_neighbors_within(&sim.bodies, i, solvation_range)
        } else {
            sim.quadtree
                .find_neighbors_within(&sim.bodies, i, solvation_range)
        };
...
fn calculate_solvation_forces_optimized(
    body: &crate::body::Body,
    all_bodies: &[crate::body::Body],
    neighbors: &[usize],
) -> f32 {
...
fn calculate_density_gradient_force_optimized(
    body: &crate::body::Body,
    all_bodies: &[crate::body::Body],
    neighbors: &[usize],
) -> f32 {
