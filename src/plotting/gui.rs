...
use crate::plotting::{ExportFormat, PlotConfig, PlotType, PlottingSystem, Quantity, SamplingMode};
use quarkstrom::egui;
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    for window_id in window_ids {
                        if let Some(window) = plotting_system.windows.get(&window_id) {
                            let button_text =
                                format!("{} ({:?})", window.config.title, window.config.plot_type);
            ...
            ui.horizontal(|ui| {
                ...
                egui::ComboBox::from_id_source("sampling_mode")
                    ...
                    .show_ui(ui, |ui| {
                        ...
                        ui.selectable_value(
                            new_plot_sampling_mode,
                            SamplingMode::TimeAveraged { window: 1.0 },
                            "Time Averaged",
                        );
                    });
            });
...
fn show_quantity_selector(ui: &mut egui::Ui, quantity: &mut Quantity, plot_type: &PlotType) {
    ...
    if !is_quantity_compatible_with_plot_type(quantity, plot_type) {
        ...
        *quantity = match plot_type {
            PlotType::TimeSeries => Quantity::TotalSpeciesCount(Species::LithiumIon),
            _ => Quantity::Charge,
        };
    }
    ...
    egui::ComboBox::from_id_source("quantity")
        ...
        .show_ui(ui, |ui| {
            ...
            ui.selectable_value(
                quantity,
                Quantity::TotalSpeciesCount(Species::LithiumMetal),
                "Li Metal Count",
            );
            ui.selectable_value(
                quantity,
                Quantity::TotalSpeciesCount(Species::ElectrolyteAnion),
                "Anion Count",
            );
...
fn show_plot_content(ui: &mut egui::Ui, window: &mut crate::plotting::PlotWindow) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.label(format!(
            "Update freq: {:.1} Hz",
            window.config.update_frequency
        ));
    ...
    if window.data.x_data.is_empty() {
        ...
        if ui.is_rect_visible(rect) {
            ...
            for point in &screen_points {
                ui.painter()
                    .circle_filled(*point, 2.0, egui::Color32::from_rgb(0, 100, 255));
            }
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button("Export CSV").clicked() {
            if let Ok(path) = crate::plotting::export::export_plot_data(
                &window.data,
                crate::plotting::ExportFormat::CSV,
            ) {
        ...
        if ui.button("Export JSON").clicked() {
            if let Ok(path) = crate::plotting::export::export_plot_data(
                &window.data,
                crate::plotting::ExportFormat::JSON,
            ) {
                ui.label(format!("Exported to: {}", path));
            }
        }
        ...
        if ui.button("Export TSV").clicked() {
            if let Ok(path) = crate::plotting::export::export_plot_data(
                &window.data,
                crate::plotting::ExportFormat::TSV,
            ) {
                ui.label(format!("Exported to: {}", path));
            }
        }
...
fn calculate_plot_ranges(
    config: &crate::plotting::PlotConfig,
    data: &crate::plotting::PlotData,
) -> (f64, f64, f64, f64) {
