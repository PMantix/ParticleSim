...
impl super::super::Renderer {
        ...
        egui::Window::new("📈 PID Controller Graph")
            ...
            .show(ctx, |ui| {
                ...
                if master_foils.is_empty() {
                    ui.label(
                        "No master foils with overpotential control and history data available.",
                    );
                ...
                ui.horizontal(|ui| {
                    ...
                    egui::ComboBox::from_id_source("pid_graph_foil_selector")
                        .selected_text(format!(
                            "Foil {}",
                            self.selected_pid_foil_id.unwrap_or(master_foils[0].id)
                        ))
                        .show_ui(ui, |ui| {
                            for foil in &master_foils {
                                ui.selectable_value(
                                    &mut self.selected_pid_foil_id,
                                    Some(foil.id),
                                    format!("Foil {}", foil.id),
                                );
                            }
                    ...
                    if self.selected_pid_foil_id.is_none()
                        || !master_foils
                            .iter()
                            .any(|f| Some(f.id) == self.selected_pid_foil_id)
                    {
    ...
    fn draw_pid_plot(
        ...
        let setpoint_min = history
            .iter()
            .map(|p| p.setpoint)
            .fold(f32::INFINITY, f32::min) as f64;
        let setpoint_max = history
            .iter()
            .map(|p| p.setpoint)
            .fold(f32::NEG_INFINITY, f32::max) as f64;
        let actual_min = history
            .iter()
            .map(|p| p.actual)
            .fold(f32::INFINITY, f32::min) as f64;
        let actual_max = history
            .iter()
            .map(|p| p.actual)
            .fold(f32::NEG_INFINITY, f32::max) as f64;
        ...
        let error_min = history
            .iter()
            .map(|p| p.error)
            .fold(f32::INFINITY, f32::min) as f64;
        let error_max = history
            .iter()
            .map(|p| p.error)
            .fold(f32::NEG_INFINITY, f32::max) as f64;
        let output_min = history
            .iter()
            .map(|p| p.output)
            .fold(f32::INFINITY, f32::min) as f64;
        let output_max = history
            .iter()
            .map(|p| p.output)
            .fold(f32::NEG_INFINITY, f32::max) as f64;
        ...
        self.draw_simple_plot(
            ...
            y_max3,
            &[
                ("P Term", egui::Color32::from_rgb(255, 100, 100), |p| {
                    p.p_term as f64
                }),
                ("I Term", egui::Color32::from_rgb(100, 100, 255), |p| {
                    p.i_term as f64
                }),
                ("D Term", egui::Color32::from_rgb(100, 255, 100), |p| {
                    p.d_term as f64
                }),
            ],
        );
        ...
        if let Some(latest) = history.back() {
            ...
            ui.horizontal(|ui| {
                ...
                ui.colored_label(
                    egui::Color32::BLACK,
                    format!("Setpoint: {:.3}", latest.setpoint),
                );
                ui.colored_label(
                    if latest.error.abs() < 0.01 {
                        egui::Color32::GREEN
                    } else {
                        egui::Color32::RED
                    },
                ...
                ui.colored_label(
                    ...
                    format!("Output: {:.2}A", latest.output),
                );
            });
        }
    }
    ...
    fn draw_simple_plot(
        ...
        series: &[(
            &str,
            egui::Color32,
            fn(&crate::body::foil::PidHistoryPoint) -> f64,
        )],
    ) {
        let (rect, _response) =
        ...
        if ui.is_rect_visible(rect) {
            ...
            for (name, color, _) in series {
                painter.line_segment(
                    [
                        egui::pos2(rect.max.x - 80.0, legend_y),
                        egui::pos2(rect.max.x - 60.0, legend_y),
                    ],
                    egui::Stroke::new(2.0, *color),
                );
                painter.text(
