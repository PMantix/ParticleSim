...
impl super::Renderer {
    pub fn show_gui(&mut self, ctx: &quarkstrom::egui::Context) {
        egui::Window::new("")
            ...
            .show(ctx, |ui| {
                egui::CollapsingHeader::new("Field Controls")
                    .default_open(true)
                    .show(ui, |ui| {
                ...
                egui::CollapsingHeader::new("Visualization Overlays")
                    .default_open(true)
                    .show(ui, |ui| {
                ...
                egui::CollapsingHeader::new("Lennard-Jones Parameters")
                    .default_open(true)
                    .show(ui, |ui| {
                        ui.add(
                            egui::Slider::new(&mut self.sim_config.lj_force_epsilon, 0.0..=5000.0)
                                .text("LJ Epsilon (attraction strength)")
                                .step_by(1.0),
                        );
                        ui.add(
                            egui::Slider::new(&mut self.sim_config.lj_force_sigma, 0.1..=5.0)
                                .text("LJ Sigma (particle size)")
                                .step_by(0.01),
                        );
                        ui.add(
                            egui::Slider::new(&mut self.sim_config.lj_force_cutoff, 0.5..=10.0)
                                .text("LJ Cutoff (range factor)")
                                .step_by(0.01),
                        );
                        if ui.button("Reset LJ Parameters").clicked() {
                            self.sim_config.lj_force_epsilon = config::LJ_FORCE_EPSILON;
                            self.sim_config.lj_force_sigma = config::LJ_FORCE_SIGMA;
                            self.sim_config.lj_force_cutoff = config::LJ_FORCE_CUTOFF;
                        }
                    });
