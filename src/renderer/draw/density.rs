...
impl Renderer {
    ...
    pub fn calculate_2d_domain_density(
        &self,
        grid_spacing: f32,
        smoothing: f32,
    ) -> (Vec<f32>, f32, usize, usize, Vec2, Vec2) {
    ...
    pub fn calculate_numerical_density(&self) -> (f32, usize, f32) {
        ...
        let selected_particles: Vec<&crate::body::Body> = self
            .bodies
            .iter()
        ...
        let total_particle_area: f32 = selected_particles
            .iter()
        ...
        let effective_area = if effective_area > 0.0 {
            effective_area
        } else {
            1.0
        };
    ...
    pub fn draw_2d_domain_density(&self, ctx: &mut quarkstrom::RenderContext) {
        ...
        let (samples, max_density, nx, ny, min, _max) =
            self.calculate_2d_domain_density(grid_spacing, smoothing);
        ...
        for ix in 0..nx - 1 {
            for iy in 0..ny - 1 {
                ...
                let g = ((1.0 - normalized_density) * normalized_density * 4.0 * 255.0).min(255.0)
                    as u8; // Peak at middle density
