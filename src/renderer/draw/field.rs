    ...
    let dist_gamma = self.sim_config.isoline_distribution_gamma.max(0.001);
        ...
        if self.sim_config.isoline_filled && iso_values.len() >= 2 {
            ...
            for band in 0..iso_values.len()-1 {
                ...
                let mid = 0.5*(v0+v1);
                let mut color = map_iso_color(mid, abs_max, self.sim_config.isoline_color_strength, self.sim_config.isoline_color_gamma);
                color[3] = alpha;
                ...
                for ix in 0..nx - 1 {
                    for iy in 0..ny - 1 {
                        ...
                        let vmin = field_grid[i00].min(field_grid[i10]).min(field_grid[i01]).min(field_grid[i11]);
                        let vmax = field_grid[i00].max(field_grid[i10]).max(field_grid[i01]).max(field_grid[i11]);
                        if vmin <= v1 && vmax >= v0 { // cell intersects band
        ...
        for &iso in &iso_values {
            let mut color = map_iso_color(iso, abs_max, self.sim_config.isoline_color_strength, self.sim_config.isoline_color_gamma);
            color[3] = 255;

            // Optionally refine a band of iso levels
            let level_t = if num_levels > 1 { iso_values.iter().position(|v| *v == iso).unwrap_or(0) as f32 / (num_levels - 1) as f32 } else { 0.0 };
            let refine_enabled = self.sim_config.isoline_local_refine
                && self.sim_config.isoline_local_refine_factor > 1
                && (level_t - 0.5).abs() <= (self.sim_config.isoline_local_refine_band.clamp(0.1, 1.0) * 0.5);
                    ...
                    if refine_enabled && coarse_crosses {
                        ...
                        for sx in 0..n {
                            for sy in 0..n {
                                ...
                                let pp00 = Vec2::new(p00.x + (p10.x - p00.x) * fx0 + (p01.x - p00.x) * fy0,
                                                     p00.y + (p10.y - p00.y) * fx0 + (p01.y - p00.y) * fy0);
                                let pp10 = Vec2::new(p00.x + (p10.x - p00.x) * fx1 + (p01.x - p00.x) * fy0,
                                                     p00.y + (p10.y - p00.y) * fx1 + (p01.y - p00.y) * fy0);
                                let pp01 = Vec2::new(p00.x + (p10.x - p00.x) * fx0 + (p01.x - p00.x) * fy1,
                                                     p00.y + (p10.y - p00.y) * fx0 + (p01.y - p00.y) * fy1);
                                let pp11 = Vec2::new(p00.x + (p10.x - p00.x) * fx1 + (p01.x - p00.x) * fy1,
                                ...
                                if sub_pts.len() == 2 { ctx.draw_line(sub_pts[0], sub_pts[1], color); }
                            }
                        }
                    } else {
                        let mut pts = Vec::new();
                        ...
                        if pts.len() == 2 { ctx.draw_line(pts[0], pts[1], color); }
                    }
                }
            }
        }
...
fn map_iso_color(value: f32, abs_max: f32, strength: f32, gamma: f32) -> [u8;4] {
    ...
    let sign = if t < 0.0 { -1.0 } else if t > 0.0 { 1.0 } else { 0.0 };
