...
pub fn process_command(cmd: SimCommand, simulation: &mut Simulation) {
    match cmd {
        ...
        SimCommand::SetOutOfPlane { enabled, max_z, z_stiffness, z_damping, enable_solvent, vis_min_frac, vis_max_frac } => {
            state::handle_set_out_of_plane(simulation, enabled, max_z, z_stiffness, z_damping, enable_solvent, vis_min_frac, vis_max_frac);
        }
    }
}
