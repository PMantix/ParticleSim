...
pub fn export_results_to_csv(
    ...
    for time_key in timestamps {
        ...
        let sample_map: HashMap<_, _> = group
            .iter()
    ...
    println!(
        "✓ Exported results for case {} to {}",
        case.case_id, filename
    );
    Ok(())
}
...
pub fn export_doe_summary(
    ...
    for (case, samples) in cases.iter().zip(all_samples.iter()) {
        ...
        let final_samples: Vec<_> = samples
            .iter()
            .filter(|s| s.time_fs >= samples.last().map(|l| l.time_fs - 1000.0).unwrap_or(0.0))
            .collect();

        let final_li_metal_count: usize = final_samples
            .iter()
            .map(|s| s.lithium_metal_count)
            .sum::<usize>()
            / final_samples.len().max(1);

        let avg_edge_position: f32 = final_samples
            .iter()
            .map(|s| s.lithium_metal_edge_position)
            .sum::<f32>()
            / final_samples.len() as f32;

        let max_edge_position: f32 = samples
            .iter()
