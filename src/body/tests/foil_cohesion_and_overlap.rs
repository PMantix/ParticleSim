...
mod foil_cohesion_and_overlap {
    ...
    fn overlapping_foil_indices_handled() {
        ...
        sim.foils
            .push(Foil::new(vec![id], Vec2::zero(), 1.0, 1.0, 1.0, 0.0));
        sim.foils
            .push(Foil::new(vec![id], Vec2::zero(), 1.0, 1.0, -1.0, 0.0));
        sim.step();
        assert_eq!(
            sim.bodies[idx].electrons.len(),
            crate::config::FOIL_NEUTRAL_ELECTRONS,
            "Overlapping foils should not crash and net current is zero"
        );
    }
    ...
    fn foil_particles_remain_cohesive_within_electron_limits() {
        ...
        let avg_dist_after_remove: f32 = (0..n - 1)
            ...
            / (n as f32 - 1.0);
        assert!(
            avg_dist_after_add < initial_avg_dist * 1.5,
            "Foil particles separated too much after adding electrons: {} vs {}",
            avg_dist_after_add,
            initial_avg_dist
        );
        assert!(
            avg_dist_after_remove < initial_avg_dist * 1.5,
            "Foil particles separated too much after removing electrons: {} vs {}",
            avg_dist_after_remove,
            initial_avg_dist
        );
    }
}
