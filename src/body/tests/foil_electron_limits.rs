...
mod foil_electron_limits {
    ...
    fn foil_does_not_drop_below_zero_electrons() {
        ...
        assert_eq!(
            ...
            0,
            "Foil should be able to go down to 0 electrons"
        );
        assert_eq!(
            sim.bodies[1].electrons.len(),
            crate::config::FOIL_NEUTRAL_ELECTRONS + 1,
            "Charge conservation foil should gain the transferred electron"
        );
    }
    ...
    fn foil_current_adds_and_removes_electrons_within_limits() {
        ...
        foil1.accum =
            (crate::config::FOIL_MAX_ELECTRONS - crate::config::FOIL_NEUTRAL_ELECTRONS) as f32; // Wants to gain 1 electron
        sim.foils.push(foil1);

        // Charge conservation foil that loses electrons
        let mut foil2 = Foil::new(vec![id2], Vec2::new(10.0, 0.0), 1.0, 1.0, -2.0, 0.0);
        foil2.accum =
            -((crate::config::FOIL_MAX_ELECTRONS - crate::config::FOIL_NEUTRAL_ELECTRONS) as f32); // Wants to lose 1 electron
        sim.foils.push(foil2);
        ...
        assert_eq!(
            ...
            "Electrons should be added up to FOIL_MAX_ELECTRONS"
        );
        assert_eq!(
            sim.bodies[1].electrons.len(),
            crate::config::FOIL_NEUTRAL_ELECTRONS - 1,
            "Charge conservation foil should lose one electron"
        );

        // Test removing electrons (reverse the accumulations)
        sim.foils[0].dc_current = -2.0;
        sim.foils[0].accum =
            -((crate::config::FOIL_MAX_ELECTRONS - crate::config::FOIL_NEUTRAL_ELECTRONS) as f32); // Now wants to lose electrons
        sim.foils[1].dc_current = 2.0;
        sim.foils[1].accum =
            (crate::config::FOIL_MAX_ELECTRONS - crate::config::FOIL_NEUTRAL_ELECTRONS) as f32; // Now wants to gain electrons

        sim.step();
        assert_eq!(
            sim.bodies[0].electrons.len(),
            crate::config::FOIL_NEUTRAL_ELECTRONS,
            "Electrons should be removed back to neutral level"
        );
        assert_eq!(
            sim.bodies[1].electrons.len(),
            crate::config::FOIL_NEUTRAL_ELECTRONS,
            "Charge conservation foil should gain back the electron"
        );
    }
    ...
    fn foil_default_electrons() {
        ...
        assert_eq!(
            body.electrons.len(),
            crate::config::FOIL_NEUTRAL_ELECTRONS,
            "FoilMetal should start with correct number of electrons"
        );
    }
    ...
    fn linked_foils_stop_when_one_cannot_transfer() {
        ...
        assert_eq!(
            sim.bodies[0].electrons.len(),
            crate::config::FOIL_NEUTRAL_ELECTRONS,
            "Foil A should not gain electrons when Foil B cannot lose any"
        );
        assert_eq!(
            sim.bodies[1].electrons.len(),
            0,
            "Foil B should remain at 0 electrons since it cannot lose any more"
        );
    }
}
